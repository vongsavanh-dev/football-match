...
<script>
import AddMatch  from "@/views/Match/CRUD/AddMatch";
import EditMatch  from "@/views/Match/CRUD/EditMatch";
import DeleteMatch from "@/views/Match/CRUD/DeleteMatch";
...
export default {
  ...
  methods: {
    ...
    FetchData() {
      this.$axios.get('match-standing').then(res => {
        // console.log(res)
        setTimeout(() => {
          this.$emit('close');
        }, 200);
