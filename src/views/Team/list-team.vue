<template>
  <div class="container">
      ...
      <vs-table v-model="selected">
        <template #thead>
          <vs-tr class="table-header">
            <vs-th id="table-checkbox">
              <vs-checkbox
                  :indeterminate="selected.length == teams.length"
                  v-model="allCheck"
                  @change="selected = $vs.checkAll(selected, teams)"
              />
            </vs-th>
        ...
        <template #tbody>
          <vs-tr :key="index" v-for="(team, index) in $vs.getPage(teams, page, max)" :data="team"
                 :is-selected="!!selected.includes(team)">
            <vs-td checkbox>
              <vs-checkbox :val="team" v-model="selected"/>
            </vs-td>
            <vs-td>
              {{ index + 1 }}
            </vs-td>
    ...
    <TeamExport v-show="false" ref="teamPdfPrint"></TeamExport>
...
import TeamExport from "@/components/Export/TeamExport"
export default {
  components: {
    AddTeam,
    EditTeam,
  ...
  methods: {
    ...
    PrintExcel(selected) {
      const ref = this.$refs["teamPdfPrint"];
      if (ref) {
